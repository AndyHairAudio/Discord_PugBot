...
var commands = {
	...
	"startctfpug": {
        usage: "<pugname>",
        description: "start a pug for users to join",
        process: function(bot,msg,suffix){
			bot.sendMessage(msg.channel, "Starting startpug");
			var type = 'Capture The Flag';
			var startauthor = msg.author;
			var users = [startauthor];
			var id = suffix;
			var freshpug = new puginstance(type, startauthor, id, users);
			activePugs.push(freshpug);
			var out = startauthor + " has started a CTF pug, type '!joinctfpug " + suffix + "' to join. [1/10]";
			bot.sendMessage(msg.channel,out);
			bot.sendMessage(msg.channel, "Finished startpug");
		}
    },
	"joinctfpug": {
        usage: "<pugname>",
        description: "join an existing ctf pug",
        process: function(bot,msg,suffix){
			bot.sendMessage(msg.channel, "Starting joinpug");
			var entry, pugtype, userentry, pugid, pugusers, id, joinauthor;
			joinauthor = msg.author;
			for (i = 0; i < activePugs.length; i++){
				id = suffix;
				entry = activePugs[i];
				pugid = entry.pugid;
				pugusers = entry.pugusers;
				if (pugid == id){
					for (x = 0; x < pugusers.length; x++){
						userentry = pugusers[x];
						if(userentry == joinauthor) {
							bot.sendMessage(msg.channel, "You have already signed up for this pug!");
							x=pugusers.length;
							i=activePugs.length;
						}
						else {
							activePugs[i].pugusers.push(joinauthor);
							bot.sendMessage(msg.channel, joinauthor + " has joined CTF pug " + suffix + " [" + entry.pugusers.length + "/10]");
							x=pugusers.length;
							i=activePugs.length;
						}
					}
					...
					bot.sendMessage(msg.channel, "The pug: " + id + " does not exist!");
				}
			}
			bot.sendMessage(msg.channel, "Finished joinpug");
		}
    },
	"activepugs": {
        description: "list active pugs",
        process: function(bot,msg){
			var entry, type, creator, id, users;
			bot.sendMessage(msg.channel, "Active PuGs: ");
			for (b = 0; b < activePugs.length; b++) {
				entry = activePugs[b];
				type = entry.pugtype;
				creator = entry.pugcreator;
				id = entry.pugid;
				users = entry.pugusers;
				bot.sendMessage(msg.channel, "Gametype: " + type + " | Creator: " + creator + " | ID: " + id + " | Players: " + users);
			}
		}
    }
}
...
function puginstance(type, creator, id, users) {
	this.pugtype = type;
	this.pugcreator = creator;
	this.pugid = id;
	this.pugusers = users;
}
